...
def lvl_dos_ejecu():
    ...
    fondo = pygame.transform.scale(pygame.image.load("imagenes\_1_game_background\_2_game_background.png"),(1200,800))
    x_para_fondo = 0
    FPS = 25
    ...
    while running_flag:
        ...
        if personaje.hp <= 0:
            running_flag = False
            retorno =  "perdio"
            actualizar_ranking(personaje.puntuacion + 4)
        ...
        if personaje.contador_kills ==  1:
            sangre_posicion_zombie = (zombie.zx+40, zombie.zy+270)
            zombie.kill()
            zombie = ZombieDos()
            lista_zombies.add(zombie)
